...
# Creating dictionary with keys as class name and value as class model 
audioTypes={model.Config.classname:model for model in audio_types}
...
def create(data:AudioCreate):
    ...
    '''
    try:
        model=audioTypes[data.audioType.lower()]
    except:
...
def delete(audio_type:str,audio_id:str):
    ...
    '''
    try:
        model=audioTypes[audio_type.lower()]
    except:
